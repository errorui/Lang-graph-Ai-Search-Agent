...
const MessageArea = ({ messages }:{
    ...
    return (
        <div className="flex-grow overflow-y-auto bg-[#FCFCF8] border-b " style={{ minHeight: 0 }}>
                        ...
                        <div className="flex flex-col max-w-md">
                            ...
                            <div
                                className={`rounded-lg py-3 px-5 ${message.isUser
                                    ? 'bg-gradient-to-br from-slate-800 to-slate-950 text-white rounded-br-none shadow-md'
                                    : 'bg-[#F3F3EE] text-gray-800 border border-slate-500 '
                                    }`}
                            >
                                {/* Only show typing animation if isLoading is true AND content is empty */}
                                {message.isLoading && message.content === "" ? (
                                    <PremiumTypingAnimation />
                                ) : (
                                    // Display content if available, otherwise "Waiting..." if still loading or if content is explicitly empty
                                    message.content || (message.isLoading ? <span className="text-gray-400 text-xs italic">Generating response...</span> : "")
                                )}
                            </div>
                        </div>
                    </div>
